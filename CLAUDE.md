...
8. **THE SAME IS TRUE OF TABLES.** A new table in `public` comes out `anon=arwdDxtm` — full SELECT/INSERT/UPDATE/DELETE. **Every migration creating a table in `public` carries an explicit `revoke all on table <t> from anon`.**
   **AMENDED 2026-09-14 — AND THE `anon` REVOKE ALONE IS WHY THE SWEEPS MISSED A LIVE HOLE. A NEW TABLE'S GRANT SET IS MATCHED TO THE HOUSE STANDARD FOR BOTH ROLES, NOT JUST `anon`.** The same default hands `authenticated` `arwdDxtm` — including **TRUNCATE, which RLS cannot mediate**. The 2026-08-29..31 sweeps measured `anon`; the three purchase-order tables were created on 2026-09-07 and kept `authenticated=arwdDxtm`. **Proved 2026-09-14 (rolled back): a Material Matrix member who SELECTs 0 PO rows through RLS TRUNCATEd `purchase_order_line_promises`, 1 row → 0.** Closed by `20260915002733`. **THE HOUSE STANDARD, measured over 51 owned tables:** `authenticated` = `SELECT, INSERT, UPDATE, DELETE` exactly; `anon` = nothing. **Every new table's migration revokes `TRUNCATE, REFERENCES, TRIGGER, MAINTAIN` from `authenticated` and ALL from `anon`**, and any deliberate deviation (narrower, or `audit_leads`' anonymous INSERT) is written in the migration. **Grade a table by `has_table_privilege` for both roles, never by reading `relacl` for one.** Found 8/25 on `products`, which had RLS on and every policy scoped `TO authenticated`, so nothing was readable — **the defect was REACHABILITY, not disclosure**, and it surfaced as `pg_graphql_anon_table_exposed` moving 67 → 68. Scope new policies `TO authenticated` too: a policy left `TO public` whose expression calls a definer helper must be *evaluated* for `anon`, which is what took the Material Matrix storefront down on 8/20.
...
15. **A MIGRATION THAT SUCCEEDS AND BREAKS SOMETHING IS INDISTINGUISHABLE FROM ONE THAT SUCCEEDS, WHEN VALIDATION IS DEFERRED TO FIRST CALL.** *(Found 2026-09-03 on A2.3, by a probe of our own that was itself incomplete — which is the only reason it surfaced.)*
    ...
    **ADDING A COLUMN IS IN SCOPE TOO (2026-09-12, on our own migration).** A function declared `RETURNS SETOF <table>` that projects columns **by name** (to mask money) is bound to the row type only at call time. `collapsed_state` added three columns to `products`; `list_products` and `fetch_product` then failed `42804 … returned columns (12) does not match expected column count (15)` — the catalog and the estimate picker's product list were down ~6 minutes until a corrective applied. **Found only because the rule-15 calls were made.** The corrective's own in-migration check called both **as `postgres`**, where `my_org_ids()` is empty and both `return` before the `RETURN QUERY` — **an empty instrument written into a ledger comment that says otherwise.** Call as a member. `supabase/sweeps/collapsed_state_sweep.sql` §4 lists every by-name `SETOF <table>` function.
    **AND READ THE FORM BEFORE READING A PATCH KEY AS INTENT.** The same day, two more of our own defects passed every probe written from the RPC's point of view and failed the moment the probe was written from the FORM's: the catalog edit form always sends `sell` (blank as `null`), and the material row always resends its prefilled `ready_by`. **A resubmitted value is not a deliberate write.** Probe with the exact payload the caller builds.

16. **A CHANGE TO WHAT A FUNCTION RETURNS NAMES ITS READERS, AND RUNS `supabase/sweeps/rowtype_reader_sweep.sql` BEFORE IT APPLIES.** *(Controller ruling, 2026-09-17.)* Narrowing a return is the same act as narrowing what a function accepts. `20260918003303` narrowed `signed_copy_by_link` without naming its readers and broke two, one live: link-signed copies keyed Resend on `signed-copy:undefined`, so a second customer within 24 hours signed and received nothing. Both were caught by another track happening to read the code. Directive §7.1 has the record.

17. **ACCEPTED ON THE RENDERED SCREEN, OR NOT ACCEPTED.** *(Controller ruling, 2026-09-20, from the first read-only browser session as a real `field` member.)* A policy-reading method is sound for what a policy GOVERNS and blind to what a PERSON SEES: every defect measured in the browser that day sat behind **correct** policy. Track U refused twice in one week to score the field surface from source and was right both times. **Until the pilot (2026-10-07), a Field item is accepted on the rendered screen — not on a passing probe, a green sweep, or a correct policy.** Directive §7.1 RULE 15.

18. **A SWEEP IS ONLY AS WIDE AS THE AXIS IT WAS WRITTEN ON — STATE THE AXIS WITH THE RESULT.** *(Controller ruling, 2026-09-20.)* A prose sweep keyed on capability identifiers returned **0 across 45 crew-reachable modules** while production held three capability identifiers (`view_estimates`, `view_master_work_order`, "role: field" in a subtitle) and one module identifier ("once coordination schedules a crew"). **Correct on its axis, run on the wrong container, and it PASSED** — and a pass reads as "no defects" when it meant "no coverage". Every sweep result names its axis and what that axis cannot see. Same family as rule 9 (the advisor), rule 11's `not\s+exists` regex, and the same-transaction timestamp probe. Directive §7.1 RULE 16.
