-- ROLLBACK for 20260923 refusal_hints. Restores the seventeen bodies exactly as they were before the hints
-- were added (captured from pg_get_functiondef immediately before the migration applied), including
-- `not_found` as one code for four different sentences. Signatures and grants are untouched either way.
