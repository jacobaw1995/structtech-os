...
**Version:** 1.7 · **Updated:** 2026-08-19
...
| **Stage** | A1 — Job Spine |
| **Last completed task** | **A1.3b — RPC re-pointing: children — CLOSED (2026-08-19)** — migration `20260819002237_a1_3b_child_rpc_repoint_trade_and_master` applied via MCP. **Function DDL only; no table and no row changed.** First statement dropped the A1.2 shim `create_work_order_from_estimate` (signature copied verbatim from `pg_get_function_identity_arguments()`, bare DROP not `IF EXISTS`, zero repo callers confirmed by grep, 0 remaining verified in `pg_proc`). Four functions re-pointed to **trade** (`add_material_item`, `add_schedule_block`, `create_check_in`, `get_or_create_production_packet`), three to **master** (`record_work_order_sign_off`, `create_work_order_agreement`, `fetch_work_order_agreement`), and `update_material_item`/`delete_material_item` resolve their level **through the parent** with no work-order argument added. One shared `assert_work_order_level()` carries the check so the nine cannot drift apart. **Three silent breakages were caught and fixed in the same migration** — the after-sign-off audit trail would have gone dead, the agreement snapshot would have serialised an empty material list, and the material child RPCs checked only the item's own org. Overload count verified at exactly 1 for all nine plus both helpers. UI half shipped in the same commit: child-object forms render on a trade and not on a master, sign-off on a master and not on a trade. Verified BOTH directions by rolled-back authenticated-role probe — four positives on a trade, sign-off + agreement + fetch positives on a master, seven refusals, after-sign-off audit landing on the master. Zero residue. Advisors **0 ERROR**. `tsc --noEmit` and `next lint` clean. |
| **Previously completed** | **A1.3a — Trade surface on the master — CLOSED (2026-08-18)** — **UI only: no migration written, no schema object touched, no production row created or modified.** Master work order page lists the trades on its job (trade, assignee, predecessor), each row linking to its own page, above a create-trade form calling `create_trade_work_order` via a new `createTradeWorkOrder` server action. `trade` is **free text with a datalist built from the org's own prior trade names** — never a fixed dropdown (§10, 2026-08-17); `assignee_type` is a closed list only because `work_orders_assignee_type_check` makes it one. RPC errors surfaced verbatim, no client-side re-implementation of the rules. Trade pages offer neither the form nor the list — trades do not nest — and get an "↑ Master work order" link instead. Verified by authenticated-role probe in a rolled-back DO block: both production masters render the zero-trade empty state; two trades created appeared in the list with correct assignee/predecessor and their own pages loaded via `fetch_work_order`; all five refusal cases fired with readable messages; **zero residue, no test data on real BMR records.** Also fixed `database.types.ts`, stale since A1.1. `tsc --noEmit` and `next lint` clean. **Browser-level confirmation as a logged-in user is outstanding — no app credentials on this machine — and is what the `os.structtek.com` deploy check covers before A1.3b.** |
...
| **NEXT STEP** | **A1.4 — Hierarchy lifecycle** (§5.1). Not gated on anything in the database. **One live-deploy caveat:** the A1.3b migration is already in prod while the matching UI ships in the A1.3b commit on `main` — until Vercel deploys it, the deployed master page still renders material and schedule forms that the RPCs now refuse. Affected surface is at zero rows, but confirm the deploy before treating the master page as correct. |
| **Carried debt from A1.3b** | **Three items.** (1) **The A1.3a delete stopgap still stands and is still only a hidden button** — `delete_work_order` remains unaware of children, and A1.3b widened the blast radius by making materials, schedule blocks, check-ins and packets reachable under a master's trades. A direct RPC call still orphans all of it. **A1.4 is the fix.** (2) **`work_order_activity` now carries master-scoped rows describing trade-level changes**, with the trade named inside the text value rather than in a column. That is deliberate (the stale document is the master's) but it means "which trade changed?" is only answerable by parsing a string. If that becomes a real query, it wants a `subject_work_order_id` column — not built, not needed yet. (3) **`update_material_item` and `delete_material_item` enforce org but NOT level.** They rely on the invariant that no material item can exist on a master, because `add_material_item` refuses to create one. **That invariant holds today and the design is right** — a child id should resolve its level through its parent, not take a level argument. But the invariant is *implicit, not enforced*: a row placed on a master by a service-role script or a future backfill would be freely editable and deletable by these two. **A1.4's INHERIT pass covers 13 more functions with exactly this shape, and this is the distinction it must draw per function: "inherits its level from its parent" and "assumes an invariant nothing enforces" look identical from the outside.** Deciding which of the 13 is which is the work, not confirming they compile. |
