...
| **Stage** | A1 — Job Spine |
| **Last completed task** | **A1.1 — Job spine schema (2026-08-16)** — migration `20260816172215_a1_1_job_spine_work_order_hierarchy` applied via MCP. `work_orders_estimate_id_key` dropped (§4.4); `jobs` created with org-scoped RLS; `work_orders` extended with `job_id`, `kind`, `trade`, `assignee_type`, `assignee_ref`, `predecessor_id`; both existing work orders backfilled as `kind='master'` against derived jobs; `job_id` NOT NULL; one-master-per-job enforced. All three *Done when* checks verified; advisors clean of ERROR-level findings. |
| **Outstanding from Foundation** | (1) ✅ Directive landed at `docs/STRUCTTECH_OS_DIRECTIVE.md` + pointer in `CLAUDE.md` (commit `d985499`). (2) CLI is authenticated to the wrong Supabase org and the project is not linked. (3) ✅ Targeted pre-A1.1 backup taken (`backups/`, 5 tables, row-count verified). (4) **A1.0 migration baseline — DEFERRED**, see §4.7; hygiene, not a blocker. (5) Cut the A1 branch. **Every migration goes through MCP `apply_migration` with a matching repo file. Never `supabase db push` or `db reset` — see §4.7.** |
| **NEXT STEP** | **A1.2 — RPC split: creation** (§5.1) |
| **Carried debt from A1.1** | `create_work_order_from_estimate` is **broken in production** — it inserts `(org_id, estimate_id)` only and now violates `job_id NOT NULL`. A1.2 replaces it. Until then, work-order creation through the deployed UI fails (CLAUDE.md migration rule 5b). |
