...
**Version:** 1.8 · **Updated:** 2026-08-19
...
| **Stage** | A1 — Job Spine |
| **Last completed task** | **A1.4 — Hierarchy lifecycle — CLOSED (2026-08-19)** — migration `20260819233735_a1_4_hierarchy_lifecycle_and_inherit_level_enforcement` applied via MCP. **UI authored FIRST, migration last, per the 2026-08-19 standing correction.** Two cascade-marker columns added (`work_orders.void_cascade_source_id`, `work_order_agreements.void_cascade_source_id` + `.void_cascade_prior_status`) — the mechanism that makes a master void reversible **in one direction only**: void marks only the trades that were live, restore clears only rows carrying that master's id, so a trade voided deliberately survives a master void-and-restore. Proved with the Monday/Tuesday case. `void_work_order` cascades master→live trades and voids the master's active agreement (prior status preserved); `restore_work_order` reverses only the cascade and **refuses a trade whose master is still voided**; `delete_work_order` **REFUSES a master with children, naming the count** (decision, controller, §10) and also refuses deleting a trade that is another trade's predecessor rather than raising a raw FK error. **`fetch_work_order` deliberately unchanged** (rule 5b — its `setof work_orders` shape is what the deployed pages read); a new `fetch_work_order_tree(uuid) returns jsonb` serves level, master id, the trade list with per-trade voided/cascade state and counts, and replaces two ad-hoc job-scoped page queries. **THE 13 INHERIT FUNCTIONS WERE NOT "VERIFY ONLY": ALL 13 CAME BACK BROKEN, NOT ASSUMES.** None of them reads the parent's `kind` at all, and the guarantor that made them look safe is bypassable — `member insert own <table>` RLS INSERT policies let an authenticated user put a check-in, material item, schedule block or production packet **straight onto a master**, probe-confirmed. All 13 now ENFORCE their level through the parent via `assert_work_order_level`, which also moves their org check from the child's own row to the parent. `update_material_item`/`delete_material_item` fixed in the same pass (15 total), clearing the A1.3b debt. Overload count verified at exactly 1 for all 21 touched functions. Verified by rolled-back authenticated-role probe: cascade void 3/3, restore 3/3, Monday/Tuesday case correct, delete refused with the count, childless-master and trade deletes still work, zero active agreements left on a voided work order, agreement reactivated to its exact prior status, **and 15/15 positive AND 15/15 negative** on the child functions. Zero residue, no test data on real BMR records. Advisors **0 ERROR**. `tsc --noEmit` and `next lint` clean. |
| **Previously completed** | **A1.3b — RPC re-pointing: children — CLOSED (2026-08-19)** — see the A1.3b row in §10 and the Tue 8/18 row in §11. Superseded here by A1.4; retained in full in git history. Before it, **A1.3a — Trade surface on the master — CLOSED (2026-08-18)** — **UI only: no migration written, no schema object touched, no production row created or modified.** Master work order page lists the trades on its job (trade, assignee, predecessor), each row linking to its own page, above a create-trade form calling `create_trade_work_order` via a new `createTradeWorkOrder` server action. `trade` is **free text with a datalist built from the org's own prior trade names** — never a fixed dropdown (§10, 2026-08-17); `assignee_type` is a closed list only because `work_orders_assignee_type_check` makes it one. RPC errors surfaced verbatim, no client-side re-implementation of the rules. Trade pages offer neither the form nor the list — trades do not nest — and get an "↑ Master work order" link instead. Verified by authenticated-role probe in a rolled-back DO block: both production masters render the zero-trade empty state; two trades created appeared in the list with correct assignee/predecessor and their own pages loaded via `fetch_work_order`; all five refusal cases fired with readable messages; **zero residue, no test data on real BMR records.** Also fixed `database.types.ts`, stale since A1.1. `tsc --noEmit` and `next lint` clean. **Browser-level confirmation as a logged-in user is outstanding — no app credentials on this machine — and is what the `os.structtek.com` deploy check covers before A1.3b.** |
| **Before that** | **A1.2 — RPC split: creation — CLOSED (2026-08-17)** — migration `20260817151709_a1_2_creation_rpc_split_job_and_trade` applied via MCP; function DDL only, no table and no row changed. `create_job_from_estimate(p_estimate_id uuid) returns jsonb` creates the job and its master and returns `{job_id, master_work_order_id}` — two rows, both ids load-bearing. `create_trade_work_order(p_master_work_order_id, p_trade, p_assignee_type, p_assignee_ref, p_predecessor_id) returns uuid` inherits `org_id`/`estimate_id`/`job_id` from the master, so a trade cannot land in the wrong org or on the wrong job; external subcontractor is a valid assignee per D1. `create_work_order_from_estimate` kept its name and `(p_estimate_id uuid) returns uuid` signature as a **deprecated shim** delegating to `create_job_from_estimate` (rule 5b — the deployed UI still called it). Overload count verified at exactly 1 for all three. Repo caught up in the same commit: migration file written from `pg_get_functiondef()`, `createWorkOrderFromEstimate` → `createJobFromEstimate` reading `master_work_order_id` off the jsonb and redirecting to the estimate with an error rather than `/coordination/undefined` when that key is absent, both importers re-pointed, both new functions added to `database.types.ts`. `tsc --noEmit` and `next lint` clean. |
...
| **NEXT STEP** | **A1.5 — Crew visibility** (§5.1). Not gated on anything in the database. **Carries two items into its scope, both in its subject matter:** (1) the §6.9 `anon` EXECUTE revoke sweep, already scheduled there, and (2) **the RLS INSERT hole A1.4 found** — `member insert own <table>` on `check_ins`, `material_items`, `schedule_blocks` and `production_packets` lets an authenticated user create a child row directly on a master, bypassing the trade-asserting RPC. A1.4 made every child function refuse such a row; the row can still be *created*. A1.5 is where RLS enforcement is the subject. **One live-deploy caveat, unchanged and now covering A1.3a, A1.3b and A1.4's UI:** browser-level confirmation on `os.structtek.com` as a logged-in user is still OUTSTANDING. |
| **Carried debt from A1.4** | **Two items, both stated as debt, not as solved.** (1) **THE RLS INSERT HOLE — new, and the most consequential finding of the day.** `member insert own check_ins` / `material_items` / `schedule_blocks` / `production_packets` have `with check (org_id in (select my_org_ids()))` and nothing else, so an authenticated user can insert a child row **directly onto a master**, bypassing the trade-asserting creation RPC entirely. Probe-confirmed 2026-08-19: `create_check_in(master)` refused, and a plain `insert into check_ins (... work_order_id = <master>)` succeeded in the same session. **This also contradicts CLAUDE.md's Supabase rule 3** ("direct `.insert()` on RLS tables fails even with a valid session"), which is true of the app's pattern but not of these policies. A1.4 closed the *consequence* — all 15 child functions now refuse a wrongly-levelled row — but not the *cause*. **Scheduled into A1.5**, where RLS enforcement is already the subject. (2) **`work_order_activity` still names the trade inside a string** — carried unchanged from A1.3b, still deliberate, still wants a `subject_work_order_id` column only if "which trade changed?" becomes a real query. |
| **Carried debt from A1.3b** | **CLEARED 2026-08-19 as distinct items.** (1) The delete stopgap is gone: `delete_work_order` now refuses a master with children and names the count, and the page's `canDelete` is a hint again rather than the only guard. (2) `update_material_item`/`delete_material_item` now enforce level — folded into A1.4's 15-function pass. (3) The `work_order_activity` string-parsing item carries forward, above. |
...
- **`anon` holds EXECUTE on 109 security-definer RPCs in `public`** (found 2026-08-17 while writing the A1.2 migration file; the count was 110 until A1.3b dropped the `create_work_order_from_estimate` shim on 2026-08-19). This is the project-wide default function ACL, applied automatically on create — no migration ever granted it deliberately. A1.3b's two new helpers, `assert_work_order_level` and `job_master_sign_off`, were explicitly revoked from `public`/`anon`/`authenticated` in the same migration and do not appear in the advisor list, so the surface shrank rather than grew. **Severity is low, not zero: the advisors report 0 ERROR, and all 109 are refused on their first statement by `my_org_ids()`, which returns nothing for an unauthenticated caller.** The grant is a surface that exists for no reason, not an open door. It is not fixed per-migration — new functions would re-acquire it from the same default — so it is **one `revoke` sweep plus an `alter default privileges` change, done in A1.5** alongside the crew-visibility work, where the RPC grant surface is already the subject.
