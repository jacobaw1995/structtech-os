...
| **5** | `default_permissions_for_role`'s `else` branch is permissive, so a future 8th role gets financials by default | §2.3 | ✅ **CLOSED 2026-09-06** | somebody adds a role to `org_members_role_check` **and gives it an explicit branch that grants something** — now a deliberate edit visible in the diff, where before it was an absence. The closed default is a SECOND barrier behind the CHECK, not the only one |
