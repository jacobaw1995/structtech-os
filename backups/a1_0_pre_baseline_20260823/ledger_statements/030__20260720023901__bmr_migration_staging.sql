...
-- and lead_appointments are explicitly OUT of scope for this pass.
--
...
-- for anon/authenticated, service_role/superuser only. Intent is to DROP
-- all 5 once the migration is loaded and verified.
...
comment on table public.migration_bmr_id_map is
  'BMR migration staging: old id -> new id per entity, the provenance/idempotency backbone for docs/migration/bmr_transform.sql. Scaffolding — drop after the migration is verified.';
