...
--
-- Found live during Chunk 5 verification: presented_total only ever
-- captured `subtotal`, never tax_amount. EstimateDocument's
-- "editedSincePresented" check (Chunk 2) compares presented_total against
-- the LIVE total (subtotal + tax) — so any estimate with a nonzero
-- tax_rate falsely showed "— edited since" in front of the customer in
-- Present Mode, even with zero edits after presenting. Same signature (no
-- overload risk), just fixing what gets captured.
