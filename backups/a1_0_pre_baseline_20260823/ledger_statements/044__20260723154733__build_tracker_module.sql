...
-- attributed, in-app source of truth. Module key is 'build' (NOT 'roadmap'
-- — that key is already taken by the client-roadmap module).
--
-- Pattern precedent: 20260722120000_tracker_module.sql. update_roadmap_fields
-- follows the jsonb-patch + allowlist convention from
-- 20260724120000_update_deal_fields_jsonb_patch.sql.
...
-- update_roadmap_fields — jsonb-patch convention: key ABSENT means untouched,
-- key PRESENT (including explicit JSON null or '') means write it.
...
comment on function public.update_roadmap_fields(uuid, jsonb) is
  'Build Tracker patch-update RPC (jsonb-patch + allowlist convention). Absent key = untouched; present key = write it, including clearing notes to null/empty.';
...
comment on function public.delete_roadmap_item(uuid) is
  'Build Tracker hard-delete RPC. No archive tier — a matrix row can just be deleted outright (unlike tracker_projects, nothing else references a roadmap_item).';
...
-- array. status = 'shipped' where phase='now', else 'planned'. sort_order
-- preserves the source array's order.
-- ============================================================================
do $$
declare
  v_org_id uuid;
